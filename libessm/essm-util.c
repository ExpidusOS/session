 ...
 * Copyright (c) 2004 Jasper Huijsmans <jasper@expidus.org>
 * All rights reserved.
 *
...
gboolean
essm_start_application (gchar      **command,
                        gchar      **environment,
                        GdkScreen   *screen,
  ...
  if (screen != NULL)
    {
      if (client_machine != NULL)
        {
          gchar *display_name =
            essm_gdk_display_get_fullname (gdk_screen_get_display (screen));
  ...
  for (; *command != NULL; ++command)
    {
      ...
      argv[argc++] = expidus_expand_variables (*command, environment);
    }
...
void
essm_place_trash_window (GtkWindow *window,
                         GdkScreen *screen,
                         gint       monitor)
{
...
gboolean
essm_strv_equal (gchar **a, gchar **b)
{
...
gchar*
essm_gdk_display_get_fullname (GdkDisplay *display)
{
  const gchar *name;
  const gchar *np;
  gchar       *hostname;
...
GdkPixbuf *
essm_load_session_preview (const gchar *name)
{
  GdkDisplay *display;
  GdkPixbuf  *pb = NULL;
  gchar *display_name;
  ...
  display_name = essm_gdk_display_get_fullname (display);
  path = g_strconcat ("sessions/thumbs-", display_name, "/", name, ".png", NULL);
  filename = expidus_resource_lookup (EXPIDUS_RESOURCE_CACHE, path);
  g_free (display_name);
  g_free (path);
...
settings_list_sessions_open_rc (void)
{
  ...
  session_file  = expidus_resource_save_location (EXPIDUS_RESOURCE_CACHE, resource_name, TRUE);
  g_free (resource_name);
  g_free (display_name);

  if (!g_file_test (session_file, G_FILE_TEST_IS_REGULAR))
    {
      g_debug ("essm_manager_load_session: Something wrong with %s, Does it exist? Permissions issue?", session_file);
      return FALSE;
    }

  rc = expidus_rc_simple_open (session_file, FALSE);
  if (G_UNLIKELY (rc == NULL))
  {
    g_warning ("essm_manager_load_session: unable to open %s", session_file);
    return FALSE;
  }
  return rc;
}
...
GList *
settings_list_sessions (ExpidusRc *rc)
{
  ...
  groups = expidus_rc_get_groups (rc);
  for (n = 0; groups[n] != NULL; ++n)
    {
      if (strncmp (groups[n], "Session: ", 9) == 0)
        {
          expidus_rc_set_group (rc, groups[n]);
          session = g_new0 (EssmSessionInfo, 1);
          session->name = g_strdup (groups[n] + 9);
          session->atime = expidus_rc_read_int_entry (rc, "LastAccess", 0);
          session->preview = essm_load_session_preview (session->name);
...
static gint
compare_session_atime (EssmSessionInfo *session1,
                       EssmSessionInfo *session2)
{
                                       ...
                                       GtkTreeView *treeview)
{
  ...
  if (!gtk_tree_selection_get_selected (selection, &model, &iter))
    {
      g_warning ("essm_chooser_get_session: !gtk_tree_selection_get_selected");
      return;
    }
  ...
  g_value_unset (&value);
  rc = expidus_rc_simple_open (session_file, FALSE);
  expidus_rc_delete_group (rc, session, FALSE);
  expidus_rc_close (rc);
  g_free (session);
