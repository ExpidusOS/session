/* $Id$ */
/*-
 * Copyright (c) 2004,2008 Brian Tarricone <kelnos@expidus.org>
...
static void
expidus_session_logout_notify_error (const gchar *message,
                                  GError      *error,
                                  gboolean     have_display)
{
  if (G_LIKELY (have_display))
    {
      expidus_dialog_show_error (NULL, error, "%s", message);
    }
  else
    {
...
main (int argc, char **argv)
{
  ...
  if (opt_version)
    {
      ...
      g_print ("%s\n\n", _("and Brian Tarricone <kelnos@expidus.org>."));
  ...
  if (proxy == NULL)
    {
      expidus_session_logout_notify_error (_("Received error while trying to log out"), err, have_display);
      g_error_free (err);
      return EXIT_FAILURE;
    }
  ...
  if (!result)
    {
      ...
      if (opt_logout)
        shutdown_type = ESSM_SHUTDOWN_LOGOUT;
      else if (opt_halt)
        shutdown_type = ESSM_SHUTDOWN_HALT;
      else if (opt_reboot)
        shutdown_type = ESSM_SHUTDOWN_REBOOT;
      else if (opt_suspend)
        shutdown_type = ESSM_SHUTDOWN_SUSPEND;
      else if (opt_hibernate)
        shutdown_type = ESSM_SHUTDOWN_HIBERNATE;
      else if (opt_hybrid_sleep)
        shutdown_type = ESSM_SHUTDOWN_HYBRID_SLEEP;
      else if (opt_switch_user)
        shutdown_type = ESSM_SHUTDOWN_SWITCH_USER;
      else
        shutdown_type = ESSM_SHUTDOWN_ASK;
