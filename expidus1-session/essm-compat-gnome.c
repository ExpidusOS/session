...
static void
essm_compat_gnome_smproxy_startup (void)
{
  Atom gnome_sm_proxy;
  Display *dpy;
  Window root;
   ...
   * gnome-smproxy in expidus1, which would cause trouble otherwise.
   */
...
static void
essm_compat_gnome_smproxy_shutdown (void)
{
  gdk_error_trap_push ();
...
void
essm_compat_gnome_startup (void)
{
  if (G_UNLIKELY (gnome_compat_started))
    return;

  essm_compat_gnome_smproxy_startup ();
...
void
essm_compat_gnome_shutdown (void)
{
  if (G_UNLIKELY (!gnome_compat_started))
    return;
  ...
  essm_compat_gnome_smproxy_shutdown ();
