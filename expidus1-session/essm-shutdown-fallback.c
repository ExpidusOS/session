 ...
 * Copyright (c) 2014      Expidus Development Team <expidus1-dev@expidus.org>
...
#include <libessm/essm-shutdown-common.h>
...
static gboolean
essm_shutdown_fallback_user_is_operator (void)
{
  struct passwd *pw;
...
#ifdef __BACKEND_TYPE_BSD__
static gboolean
essm_shutdown_fallback_bsd_check_auth (EssmShutdownType shutdown_type)
{
  ...
  switch (shutdown_type)
    ...
    case ESSM_SHUTDOWN_RESTART:
      auth_result = essm_shutdown_fallback_user_is_operator ();
      break;
...
static gboolean
essm_shutdown_fallback_check_auth_polkit (const gchar *action_id)
{
  gboolean auth_result = FALSE;
#ifdef HAVE_POLKIT
  GDBusConnection *bus;
...
lock_screen (GError **error)
{
  ...
  ExpidusScreenSaver *saver;
  gboolean       ret = TRUE;

  channel = essm_open_config ();
  saver = expidus_screensaver_new ();
  if (esconf_channel_get_bool (channel, "/shutdown/LockScreen", FALSE))
      ret = expidus_screensaver_lock (saver);
...
/**
 * essm_shutdown_fallback_try_action:
 * @type:  The @EssmShutdownType action to perform (shutdown, suspend, etc).
 * @error: Returns a @GError if an error was encountered.
 *
 * Performs the @EssmShutdownType action requested.
 *
 * Return value: Returns FALSE if an invalid @EssmShutdownType action was
 *               requested. Otherwise returns the status of the pkexec
 *               call to the helper command.
 **/
gboolean
essm_shutdown_fallback_try_action (EssmShutdownType   type,
                                   GError           **error)
{
  const gchar *essm_helper_action;
  ...
  {
    case ESSM_SHUTDOWN_SHUTDOWN:
      essm_helper_action = "shutdown";
      cmd = POWEROFF_CMD;
      break;
    case ESSM_SHUTDOWN_RESTART:
      essm_helper_action = "restart";
      cmd = REBOOT_CMD;
      break;
    case ESSM_SHUTDOWN_SUSPEND:
      essm_helper_action = "suspend";
    ...
    case ESSM_SHUTDOWN_HIBERNATE:
      ...
      break;
    case ESSM_SHUTDOWN_HYBRID_SLEEP:
      essm_helper_action = "hybrid-sleep";
      cmd = UP_BACKEND_HIBERNATE_COMMAND;
  ...
  if (essm_shutdown_fallback_bsd_check_auth (type))
    {
      return g_spawn_command_line_sync (cmd, NULL, NULL, &exit_status, error);
    }
#endif

  /* essm-shutdown-helper requires polkit to run */
#ifdef HAVE_POLKIT
  command = g_strdup_printf ("pkexec " ESSM_SHUTDOWN_HELPER_CMD " --%s", essm_helper_action);
  ...
  if (!ret)
    {
      g_set_error (error, 1, 0, "Failed to %s (%s)", essm_helper_action, cmd);
    }
  return ret;
}
...
/**
 * essm_shutdown_fallback_can_suspend:
 *
 * Return value: Returns whether the *system* is capable suspending.
 **/
gboolean
essm_shutdown_fallback_can_suspend (void)
{
#ifdef BACKEND_TYPE_FREEBSD
...
/**
 * essm_shutdown_fallback_can_hibernate:
 *
 * Return value: Returns whether the *system* is capable hibernating.
 **/
gboolean
essm_shutdown_fallback_can_hibernate (void)
{
#ifdef BACKEND_TYPE_FREEBSD
...
/**
 * essm_shutdown_fallback_can_hybrid_sleep:
 *
 * Return value: Returns whether the *system* is capable of hybrid sleep.
 **/
gboolean
essm_shutdown_fallback_can_hybrid_sleep (void)
{
#ifdef BACKEND_TYPE_FREEBSD
  return freebsd_supports_sleep_state ("S4");
#endif
...
/**
 * essm_shutdown_fallback_auth_shutdown:
 *
 * Return value: Returns whether the user is authorized to perform a shutdown.
 **/
gboolean
essm_shutdown_fallback_auth_shutdown (void)
{
#ifdef __BACKEND_TYPE_BSD__
  gboolean ret_val = essm_shutdown_fallback_bsd_check_auth (ESSM_SHUTDOWN_SHUTDOWN);
  if (ret_val)
    return TRUE;
#endif
  return essm_shutdown_fallback_check_auth_polkit (POLKIT_AUTH_SHUTDOWN_ESSM);
}



/**
 * essm_shutdown_fallback_auth_restart:
 *
 * Return value: Returns whether the user is authorized to perform a restart.
 **/
gboolean
essm_shutdown_fallback_auth_restart (void)
{
#ifdef __BACKEND_TYPE_BSD__
  gboolean ret_val = essm_shutdown_fallback_bsd_check_auth (ESSM_SHUTDOWN_RESTART);
  if (ret_val)
    return TRUE;
#endif
  return essm_shutdown_fallback_check_auth_polkit (POLKIT_AUTH_RESTART_ESSM);
}



/**
 * essm_shutdown_fallback_auth_suspend:
 *
 * Return value: Returns whether the user is authorized to perform a suspend.
 **/
gboolean
essm_shutdown_fallback_auth_suspend (void)
{
#ifdef __BACKEND_TYPE_BSD__
  gboolean ret_val = essm_shutdown_fallback_bsd_check_auth (ESSM_SHUTDOWN_SUSPEND);
  if (ret_val)
    return TRUE;
#endif
  return essm_shutdown_fallback_check_auth_polkit (POLKIT_AUTH_SUSPEND_ESSM);
}



/**
 * essm_shutdown_fallback_auth_hibernate:
 *
 * Return value: Returns whether the user is authorized to perform a hibernate.
 **/
gboolean
essm_shutdown_fallback_auth_hibernate (void)
{
#ifdef __BACKEND_TYPE_BSD__
  gboolean ret_val = essm_shutdown_fallback_bsd_check_auth (ESSM_SHUTDOWN_HIBERNATE);
  if (ret_val)
    return TRUE;
#endif
  return essm_shutdown_fallback_check_auth_polkit (POLKIT_AUTH_HIBERNATE_ESSM);
}



/**
 * essm_shutdown_fallback_auth_hybrid_sleep:
 *
 * Return value: Returns whether the user is authorized to perform a hybrid sleep.
 **/
gboolean
essm_shutdown_fallback_auth_hybrid_sleep (void)
{
#ifdef __BACKEND_TYPE_BSD__
  gboolean ret_val = essm_shutdown_fallback_bsd_check_auth (ESSM_SHUTDOWN_HYBRID_SLEEP);
  if (ret_val)
    return TRUE;
#endif
  return essm_shutdown_fallback_check_auth_polkit (POLKIT_AUTH_HYBRID_SLEEP_ESSM);
}
