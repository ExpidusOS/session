...
static gboolean essm_startup_session_next_prio_group (EssmManager *manager);

static void     essm_startup_data_free               (EssmStartupData *sdata);
static void     essm_startup_child_watch             (GPid         pid,
                                                      gint         status,
                                                      gpointer     user_data);
static gboolean essm_startup_timeout                 (gpointer     data);

static void     essm_startup_handle_failed_startup   (EssmProperties *properties,
                                                      EssmManager    *manager);
...
static pid_t
essm_gpg_agent_pid (const gchar *gpg_agent_info)
{
  pid_t        pid = -1;
  gchar      **fields;
...
static pid_t
essm_ssh_agent_pid (const gchar *ssh_agent_pid)
{
...
static pid_t
essm_startup_init_agent (const gchar *cmd,
                         const gchar *agent,
                         gboolean want_pid)
{
          ...
          if (want_pid && pid <= 0)
            {
              if (g_strcmp0 (variable, "SSH_AGENT_PID") == 0)
                pid = essm_ssh_agent_pid (value);
              else if (g_strcmp0 (variable, "GPG_AGENT_INFO") == 0)
                pid = essm_gpg_agent_pid (value);
            }
...
static void essm_gpg_agent_shutdown(gboolean quiet)
{
      GError      *error = NULL;
...
void
essm_startup_init (EsconfChannel *channel)
{
  gchar       *ssh_agent;
  ...
  if (esconf_channel_get_bool (channel, "/compat/LaunchGNOME", FALSE))
    {
  ...
  if (esconf_channel_get_bool (channel, "/startup/gpg-agent/enabled", TRUE))
    {
      gpg_agent_path = g_find_program_in_path ("gpg-agent");
      if (gpg_agent_path == NULL)
        g_printerr ("expidus1-session: %s\n",
                    "No GPG agent found");
    }

  if (esconf_channel_get_bool (channel, "/startup/ssh-agent/enabled", TRUE))
    {
      ssh_agent = esconf_channel_get_string (channel, "/startup/ssh-agent/type", NULL);
  ...
  if (G_LIKELY (ssh_agent_path != NULL || gpgagent_ssh_enabled))
    {
      agentpid = essm_ssh_agent_pid (g_getenv ("SSH_AGENT_PID"));
      ...
      if (ssh_agent_path != NULL)
        {
          ...
          running_sshagent = essm_startup_init_agent (cmd, "ssh-agent", TRUE);
          g_free (cmd);
      ...
      essm_gpg_agent_shutdown (TRUE);
        {
          ...
          envfile = expidus_resource_save_location (EXPIDUS_RESOURCE_CACHE, "gpg-agent-info", FALSE);
          ...
          /* keep this around for shutdown */
          running_gpgagent = essm_startup_init_agent (cmd, "gpg-agent",
                                                      want_pid);
...
static void
essm_startup_autostart (EssmManager *manager)
{
  essm_launch_desktop_files_on_login (FALSE);
}



void
essm_startup_foreign (EssmManager *manager)
{
  if (essm_manager_get_compat_startup(manager, ESSM_MANAGER_COMPAT_KDE))
    essm_compat_kde_startup ();

  if (essm_manager_get_compat_startup(manager, ESSM_MANAGER_COMPAT_GNOME))
    essm_compat_gnome_startup ();
}



static void
essm_startup_at_set_gtk_modules (void)
{
  const gchar  *old;
  gchar       **modules;
  guint         i;
...
static gboolean
essm_startup_at_spi_ior_set (void)
{
  Atom        AT_SPI_IOR;
  GdkDisplay *display;
  Atom        actual_type;
...
static void
essm_startup_at (EssmManager *manager)
{
  gint n, i;

  /* start at-spi-dbus-bus and/or at-spi-registryd */
  n = essm_launch_desktop_files_on_login (TRUE);

  if (n > 0)
    {
      essm_startup_at_set_gtk_modules ();

      /* wait for 2 seconds until the at-spi registered, not very nice
       * but required to properly start an accessible desktop */
      for (i = 0; i < 10; i++)
        {
          if (essm_startup_at_spi_ior_set ())
            break;
          essm_verbose ("Waiting for at-spi to register...\n");
...
void
essm_startup_begin (EssmManager *manager)
{
  /* start assistive technology before anything else */
  if (essm_manager_get_start_at (manager))
    essm_startup_at (manager);

  if (essm_manager_get_use_failsafe_mode (manager))
    essm_verbose ("Starting the session in failsafe mode.\n");
...
gboolean
essm_startup_start_properties (EssmProperties *properties,
                               EssmManager    *manager)
{
  EssmStartupData *child_watch_data;
  EssmStartupData *startup_timeout_data;
  ...
  restart_command = essm_properties_get_strv (properties, SmRestartCommand);
  argc = g_strv_length (restart_command);
  argv = g_new (gchar *, argc + 1);
  for (n = 0; n < argc; ++n)
    argv[n] = expidus_expand_variables (restart_command[n], NULL);
  argv[n] = NULL;

  current_directory = essm_properties_get_string (properties, SmCurrentDirectory);
  ...
  if (essm_is_verbose_enabled ())
    {
      gchar *command = g_strjoinv (" ", argv);
      essm_verbose ("Launching command \"%s\" with PID %d\n", command, (gint) pid);
      g_free (command);
    }
  ...
  child_watch_data = g_new0 (EssmStartupData, 1);
  ...
  child_watch_data->properties->child_watch_id =
      g_child_watch_add_full (G_PRIORITY_LOW, properties->pid,
                              essm_startup_child_watch, child_watch_data,
                              (GDestroyNotify) essm_startup_data_free);

  /* set a timeout -- client must register in a a certain amount of time
   * or it's assumed to be broken/have issues. */
  startup_timeout_data = g_new (EssmStartupData, 1);
...
void
essm_startup_session_continue (EssmManager *manager)
{
  GQueue  *pending_properties = essm_manager_get_queue (manager, ESSM_MANAGER_QUEUE_PENDING_PROPS);
  ...
  while (client_started == FALSE && g_queue_peek_head (pending_properties) != NULL)
    client_started = essm_startup_session_next_prio_group (manager);
  ...
  if (G_UNLIKELY (client_started == FALSE && g_queue_peek_head (pending_properties) == NULL))
    {
      ...
      essm_verbose ("Nothing started and nothing to start, moving to autostart items\n");
      essm_startup_autostart (manager);
      essm_manager_signal_startup_done (manager);
    }
}
...
static gboolean
essm_startup_session_next_prio_group (EssmManager *manager)
{
  GQueue         *pending_properties = essm_manager_get_queue (manager, ESSM_MANAGER_QUEUE_PENDING_PROPS);
  GQueue         *starting_properties = essm_manager_get_queue (manager, ESSM_MANAGER_QUEUE_STARTING_PROPS);
  EssmProperties *properties;
  gint            cur_prio_group;
  gboolean        client_started = FALSE;

  properties = (EssmProperties *) g_queue_peek_head (pending_properties);
  if (properties == NULL)
    return FALSE;

  cur_prio_group = essm_properties_get_uchar (properties, GsmPriority, 50);

  essm_verbose ("Starting apps in prio group %d\n (%d)", cur_prio_group, g_queue_get_length (pending_properties));

  while ((properties = g_queue_pop_head (pending_properties)))
    {
      /* quit if we've hit all the clients in the current prio group */
      if (essm_properties_get_uchar (properties, GsmPriority, 50) != cur_prio_group)
        {
      ...
      if (essm_manager_get_use_failsafe_mode (manager))
        essm_manager_increase_failsafe_pending_clients (manager);

      if (G_LIKELY (essm_startup_start_properties (properties, manager)))
        {
...
static void
essm_startup_data_free (EssmStartupData *sdata)
{
...
static void
essm_startup_child_watch (GPid     pid,
                          gint     status,
                          gpointer user_data)
{
  EssmStartupData *cwdata = user_data;
  GQueue          *starting_properties;

  essm_verbose ("Client Id = %s, PID %d exited with status %d\n",
                cwdata->properties->client_id, (gint)pid, status);

  cwdata->properties->child_watch_id = 0;
  cwdata->properties->pid = -1;

  starting_properties = essm_manager_get_queue (cwdata->manager, ESSM_MANAGER_QUEUE_STARTING_PROPS);
  if (g_queue_find (starting_properties, cwdata->properties) != NULL)
    {
      essm_verbose ("Client Id = %s died while starting up\n",
                    cwdata->properties->client_id);
      essm_startup_handle_failed_startup (cwdata->properties, cwdata->manager);
    }

  /* NOTE: cwdata->properties could have been freed by
   * essm_startup_handle_failed_startup() above, so don't try to access
...
static gboolean
essm_startup_timeout (gpointer data)
{
  EssmStartupData *stdata = data;

  essm_verbose ("Client Id = %s failed to register in time\n",
                stdata->properties->client_id);

  stdata->properties->startup_timeout_id = 0;
  essm_startup_handle_failed_startup (stdata->properties, stdata->manager);
...
static void
essm_startup_handle_failed_startup (EssmProperties *properties,
                                    EssmManager    *manager)
{
  GQueue *starting_properties = essm_manager_get_queue (manager, ESSM_MANAGER_QUEUE_STARTING_PROPS);

  essm_verbose ("Client Id = %s failed to start\n", properties->client_id);
  ...
  if (g_queue_peek_head (starting_properties) == NULL
      && essm_manager_get_state (manager) == ESSM_MANAGER_STARTUP)
    {
