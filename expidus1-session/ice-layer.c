...
ice_error_handler (IceConn ice_conn)
{
   ...
   */
  essm_verbose ("ICE connection fd = %d, ICE I/O error on connection\n",
                      ...
                      gpointer     user_data)
{
  ...
  EssmIceConnData         *icdata = user_data;

  status = IceProcessMessages (icdata->ice_conn, NULL, NULL);

  if (status == IceProcessMessagesIOError)
    {
      essm_manager_close_connection_by_ice_conn (icdata->manager,
                                                 icdata->ice_conn);
...
{
  EssmManager *manager = ESSM_MANAGER (client_data);
  GIOChannel  *channel;
  guint        watchid;
  gint         fd;
  gint         ret;
  ...
  if (opening)
    {
      EssmIceConnData *icdata = g_new(EssmIceConnData, 1);
...
ice_tmpfile (char **name)
{
  ...
  fd = g_file_open_tmp(".essm-ICE-XXXXXX", name, &error);
  if (fd < 0)
    {
