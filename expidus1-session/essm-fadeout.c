...
#ifdef GDK_WINDOWING_X11
static Window
essm_x11_fadeout_new_window (GdkDisplay *display,
                             GdkScreen *screen)
{
...
EssmFadeout*
essm_fadeout_new (GdkDisplay *display)
{
  EssmFadeout     *fadeout;
  GdkScreen       *screen;

  fadeout = g_slice_new0 (EssmFadeout);

#ifdef GDK_WINDOWING_X11
  fadeout->xdisplay = gdk_x11_display_get_xdisplay (display);
  screen = gdk_display_get_default_screen (display);
  fadeout->xwindow = GINT_TO_POINTER (essm_x11_fadeout_new_window (display, screen));
#endif
...
void
essm_fadeout_destroy (EssmFadeout *fadeout)
{
#ifdef GDK_WINDOWING_X11
  gdk_error_trap_push ();
  ...
  g_slice_free (EssmFadeout, fadeout);
}
