...
static void       essm_logout_dialog_finalize (GObject          *object);
static GtkWidget *essm_logout_dialog_button   (const gchar      *title,
                                               const gchar      *icon_name,
                                               const gchar      *icon_name_fallback,
                                               const gchar      *icon_name_fallback2,
                                               EssmShutdownType  type,
                                               EssmLogoutDialog *dialog);
...
struct _EssmLogoutDialogClass
{
  GtkDialogClass __parent__;
};

struct _EssmLogoutDialog
{
  GtkDialog __parent__;

  /* set when a button is clicked */
  EssmShutdownType  type_clicked;
  ...
  /* pm instance */
  EssmShutdown     *shutdown;
};



G_DEFINE_TYPE (EssmLogoutDialog, essm_logout_dialog, GTK_TYPE_DIALOG)



static void
essm_logout_dialog_class_init (EssmLogoutDialogClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = essm_logout_dialog_finalize;
}



static void
essm_logout_dialog_init (EssmLogoutDialog *dialog)
{
  ...
  dialog->type_clicked = ESSM_SHUTDOWN_LOGOUT;
  dialog->shutdown = essm_shutdown_get ();
  ...
  gtk_style_context_add_class (gtk_widget_get_style_context (GTK_WIDGET (dialog)), "essm-logout-dialog");
  provider = gtk_css_provider_new ();
  gtk_css_provider_load_from_data (provider, ".essm-logout-dialog { font-size: initial; }", -1, NULL);
  gtk_style_context_add_provider (gtk_widget_get_style_context (GTK_WIDGET (dialog)),
                                  GTK_STYLE_PROVIDER (provider),
                                  GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  g_object_unref (provider);

  /* load esconf settings */
  channel = essm_open_config ();
  if (essm_shutdown_can_save_session (dialog->shutdown))
    save_session = esconf_channel_get_bool (channel, "/general/SaveOnExit", TRUE);
  ...
  gtk_style_context_add_class (gtk_widget_get_style_context (GTK_WIDGET (label)), "essm-logout-label");
  ...
  gtk_style_context_add_class (gtk_widget_get_style_context (GTK_WIDGET (button_vbox)), "essm-logout-buttons");
   ...
   * Logout
   **/
  button = essm_logout_dialog_button (_("_Log Out"), "essm-logout",
                                      "system-log-out", NULL,
                                      ESSM_SHUTDOWN_LOGOUT, dialog);
   ...
   * Reboot
   **/
  if (!essm_shutdown_can_restart (dialog->shutdown, &can_restart, &error))
    {
  ...
  button = essm_logout_dialog_button (_("_Restart"), "essm-reboot",
                                      "system-reboot", NULL,
                                      ESSM_SHUTDOWN_RESTART, dialog);
   ...
   * Shutdown
   **/
  if (!essm_shutdown_can_shutdown (dialog->shutdown, &can_shutdown, &error))
    {
  ...
  button = essm_logout_dialog_button (_("Shut _Down"), "essm-shutdown",
                                      "system-shutdown", NULL,
                                      ESSM_SHUTDOWN_SHUTDOWN, dialog);
   ...
   **/
  if (esconf_channel_get_bool (channel, "/shutdown/ShowSuspend", TRUE))
    {
      if (essm_shutdown_can_suspend (dialog->shutdown, &can_suspend, &auth_suspend, &error))
        {
          if (can_suspend)
            {
              button = essm_logout_dialog_button (_("Sus_pend"), "essm-suspend",
                                                  "system-suspend", NULL,
                                                  ESSM_SHUTDOWN_SUSPEND, dialog);
   ...
   **/
  if (!upower_not_found
      && esconf_channel_get_bool (channel, "/shutdown/ShowHibernate", TRUE))
    {
      if (essm_shutdown_can_hibernate (dialog->shutdown, &can_hibernate, &auth_hibernate, &error))
        {
          if (can_hibernate)
            {
              button = essm_logout_dialog_button (_("_Hibernate"), "essm-hibernate",
                                                  "system-hibernate", NULL,
                                                  ESSM_SHUTDOWN_HIBERNATE, dialog);
   ...
   **/
  if (!upower_not_found
      && esconf_channel_get_bool (channel, "/shutdown/ShowHybridSleep", TRUE))
    {
      if (essm_shutdown_can_hybrid_sleep (dialog->shutdown, &can_hybrid_sleep, &auth_hybrid_sleep, &error))
        {
          if (can_hybrid_sleep)
            {
              button = essm_logout_dialog_button (_("H_ybrid Sleep"), "essm-hibernate",
                                                  "system-hibernate", "system-suspend-hibernate",
                                                  ESSM_SHUTDOWN_HYBRID_SLEEP, dialog);
   ...
   **/
  if (esconf_channel_get_bool (channel, "/shutdown/ShowSwitchUser", TRUE))
    {
      if (essm_shutdown_can_switch_user (dialog->shutdown, &can_switch_user, &error))
        {
          if (can_switch_user)
            {
              button = essm_logout_dialog_button (_("Switch _User"), "essm-switch-user",
                                                  "system-users", "system-users-symbolic",
                                                  ESSM_SHUTDOWN_SWITCH_USER, dialog);
   ...
   **/
  if (essm_shutdown_can_save_session (dialog->shutdown)
      && !esconf_channel_get_bool (channel, "/general/AutoSave", FALSE))
    {
...
static void
essm_logout_dialog_finalize (GObject *object)
{
  EssmLogoutDialog *dialog = ESSM_LOGOUT_DIALOG (object);

  g_object_unref (G_OBJECT (dialog->shutdown));

  (*G_OBJECT_CLASS (essm_logout_dialog_parent_class)->finalize) (object);
}



static void
essm_logout_dialog_set_mode (EssmLogoutDialog *dialog,
                             gint              mode)
{
  gint i;
...
static void
essm_logout_dialog_button_clicked (GtkWidget        *button,
                                   EssmLogoutDialog *dialog)
{
  gint *val;
...
static GtkWidget *
essm_logout_dialog_button (const gchar      *title,
                           const gchar      *icon_name,
                           const gchar      *icon_name_fallback,
                           const gchar      *icon_name_fallback2,
                           EssmShutdownType  type,
                           EssmLogoutDialog *dialog)
{
  ...
  g_return_val_if_fail (ESSM_IS_LOGOUT_DIALOG (dialog), NULL);
  ...
  *val = type;
  ...
  g_signal_connect (G_OBJECT (button), "clicked",
      G_CALLBACK (essm_logout_dialog_button_clicked), dialog);
...
static GdkPixbuf *
essm_logout_dialog_screenshot_new (GdkScreen *screen)
{
...
static void
essm_logout_dialog_screenshot_save (GdkPixbuf   *screenshot,
                                    GdkScreen   *screen,
  ...
  if (G_LIKELY (scaled != NULL))
    {
      ...
      display_name = essm_gdk_display_get_fullname (dpy);
      path = g_strconcat ("sessions/thumbs-", display_name, "/", session_name, ".png", NULL);
      filename = expidus_resource_save_location (EXPIDUS_RESOURCE_CACHE, path, TRUE);
      g_free (display_name);
      g_free (path);
      ...
      if (!filename)
        {
          g_warning ("Unable to save screenshot, "
                     "error calling expidus_resource_save_location with %s, "
...
static void
essm_logout_dialog_grab_callback (GdkSeat   *seat,
                                  GdkWindow *window,
                                  gpointer   user_data)
{
...
static gint
essm_logout_dialog_run (GtkDialog *dialog,
                        gboolean   grab_input)
{
  GdkWindow *window;
  gint       ret;
  ...
  if (grab_input)
    {
      ...
      if (gdk_seat_grab (seat, window,
                         ...
                         FALSE, NULL, NULL,
                         essm_logout_dialog_grab_callback,
...
gboolean
essm_logout_dialog (const gchar      *session_name,
                    ...
                    gboolean          accessibility)
{
  ...
  EssmFadeout      *fadeout = NULL;
  EssmLogoutDialog *essm_dialog;
  EsconfChannel    *channel = essm_open_config ();
  gboolean          autosave;
  EssmShutdown     *shutdown;
  GdkDevice        *device;
  GdkSeat          *seat;
  ...
  shutdown = essm_shutdown_get ();
  if (essm_shutdown_can_save_session (shutdown))
    autosave = esconf_channel_get_bool (channel, "/general/AutoSave", FALSE);
  else
    autosave = FALSE;
  g_object_unref (shutdown);

  /* check if we need to bother the user */
  if (!esconf_channel_get_bool (channel, "/general/PromptOnLogout", TRUE))
    {
      *return_type = ESSM_SHUTDOWN_LOGOUT;
  ...
  if (G_LIKELY (!accessibility))
    {
      ...
      /* make a screenshot */
      if (esconf_channel_get_bool (channel, "/general/ShowScreenshots", TRUE))
        screenshot = essm_logout_dialog_screenshot_new (screen);

      /* display fadeout */
      fadeout = essm_fadeout_new (gdk_screen_get_display (screen));

      dialog = g_object_new (ESSM_TYPE_LOGOUT_DIALOG,
  ...
  essm_dialog = ESSM_LOGOUT_DIALOG (dialog);

  /* set mode */
  essm_logout_dialog_set_mode (essm_dialog, MODE_LOGOUT_BUTTONS);

  result = essm_logout_dialog_run (GTK_DIALOG (dialog), !accessibility);
  ...
  if (result == GTK_RESPONSE_OK)
    {
      ...
      else if (essm_dialog->save_session != NULL)
        *return_save_session = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (essm_dialog->save_session));
      else
        *return_save_session = FALSE;

      /* return the clicked action */
      *return_type = essm_dialog->type_clicked;
    }

  if (fadeout != NULL)
    essm_fadeout_destroy (fadeout);
  ...
  if (result == GTK_RESPONSE_OK)
    {
      esconf_channel_set_string (channel, "/general/SessionName", session_name);
      esconf_channel_set_bool (channel, "/general/SaveOnExit", *return_save_session);
    }
  ...
  if (screenshot != NULL)
    {
      if (result == GTK_RESPONSE_OK)
        essm_logout_dialog_screenshot_save (screenshot, screen, session_name);
