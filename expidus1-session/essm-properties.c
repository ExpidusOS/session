...
EssmProperties*
essm_properties_new (const gchar *client_id,
                     const gchar *hostname)
{
  EssmProperties *properties;

  properties = g_slice_new0 (EssmProperties);
  ...
  properties->sm_properties = g_tree_new_full ((GCompareDataFunc) G_CALLBACK (strcmp),
                                               ...
                                               (GDestroyNotify) essm_g_value_free);
...
static gboolean
essm_properties_extract_foreach (gpointer key,
                                 gpointer value,
                                 gpointer data)
{
...
void
essm_properties_extract (EssmProperties *properties,
                         gint           *num_props,
                         SmProp       ***props)
{
  SmProp **pp;
  ...
  *props = pp = (SmProp **) malloc (sizeof (SmProp *) * g_tree_nnodes (properties->sm_properties));
  ...
  g_tree_foreach (properties->sm_properties,
                  essm_properties_extract_foreach,
                  &pp);
...
EssmProperties *
essm_properties_load (ExpidusRc      *rc,
                      const gchar *prefix)
{
#define ENTRY(name) (compose(buffer, 256, prefix, (name)))
  ...
  client_id = expidus_rc_read_entry (rc, ENTRY ("ClientId"), NULL);
  if (client_id == NULL)
    {
  ...
  hostname = expidus_rc_read_entry (rc, ENTRY ("Hostname"), NULL);
  if (hostname == NULL)
    {
  ...
  essm_verbose ("Loading properties for client %s\n", client_id);

  properties = essm_properties_new (client_id, hostname);

  for (i = 0; strv_properties[i].name; ++i)
    {
      value_strv = expidus_rc_read_list_entry (rc, ENTRY (strv_properties[i].name), NULL);
      if (value_strv)
        {
          essm_verbose ("-> Set strv (%s)\n", strv_properties[i].xsmp_name);
          /* don't use _set_strv() to avoid a realloc of the whole strv */
          value = essm_g_value_new (G_TYPE_STRV);
  ...
  for (i = 0; str_properties[i].name; ++i)
    {
      value_str = expidus_rc_read_entry (rc, ENTRY (str_properties[i].name), NULL);
      if (value_str)
        essm_properties_set_string (properties, str_properties[i].xsmp_name, value_str);
    }

  for (i = 0; uchar_properties[i].name; ++i)
    {
      value_int = expidus_rc_read_int_entry (rc, ENTRY (uchar_properties[i].name),
                                          uchar_properties[i].default_value);
      essm_properties_set_uchar (properties, uchar_properties[i].xsmp_name, value_int);
    }
...
void
essm_properties_store (EssmProperties *properties,
                       ExpidusRc         *rc,
                       const gchar    *prefix)
{
#define ENTRY(name) (compose(buffer, 256, prefix, (name)))
  ...
  expidus_rc_write_entry (rc, ENTRY ("ClientId"), properties->client_id);
  expidus_rc_write_entry (rc, ENTRY ("Hostname"), properties->hostname);
  ...
  for (i = 0; strv_properties[i].name; ++i)
    {
      ...
      if (value)
        {
          expidus_rc_write_list_entry (rc, ENTRY (strv_properties[i].name),
  ...
  for (i = 0; str_properties[i].name; ++i)
    {
      ...
      if (value)
        {
          expidus_rc_write_entry (rc, ENTRY (str_properties[i].name),
  ...
  for (i = 0; uchar_properties[i].name; ++i)
    {
      ...
      if (value)
        {
          expidus_rc_write_int_entry (rc, ENTRY (uchar_properties[i].name),
...
gint
essm_properties_compare (const EssmProperties *a,
                         const EssmProperties *b)
{
  GValue *va, *vb;
  gint ia = 50, ib = 50;
...
gint
essm_properties_compare_id (const EssmProperties *properties,
                            const gchar *client_id)
{
...
gboolean
essm_properties_check (const EssmProperties *properties)
{
...
const gchar *
essm_properties_get_string (EssmProperties *properties,
...
gchar **
essm_properties_get_strv (EssmProperties *properties,
                          const gchar *property_name)
{
  GValue *value;
...
guchar
essm_properties_get_uchar (EssmProperties *properties,
                           const gchar *property_name,
...
const GValue *
essm_properties_get (EssmProperties *properties,
                     const gchar *property_name)
{
  g_return_val_if_fail (properties != NULL, NULL);
  g_return_val_if_fail (property_name != NULL, NULL);
...
void
essm_properties_set_string (EssmProperties *properties,
                            const gchar *property_name,
                            const gchar *property_value)
{
  ...
  if (value)
    ...
    {
      value = essm_g_value_new (G_TYPE_STRING);
      g_value_set_string (value, property_value);
      g_tree_replace (properties->sm_properties,
...
void
essm_properties_set_strv (EssmProperties *properties,
                          const gchar *property_name,
...
void
essm_properties_set_uchar (EssmProperties *properties,
                           const gchar *property_name,
                           guchar property_value)
{
  ...
  if (value)
    ...
    {
      value = essm_g_value_new (G_TYPE_UCHAR);
      g_value_set_uchar (value, property_value);
      g_tree_replace (properties->sm_properties,
                      g_strdup (property_name),
                      value);
...
gboolean
essm_properties_set (EssmProperties *properties,
                     const gchar *property_name,
                     const GValue *property_value)
{
  ...
  new_value = essm_g_value_new (G_VALUE_TYPE (property_value));
...
gboolean
essm_properties_set_from_smprop (EssmProperties *properties,
                                 const SmProp *sm_prop)
{
  ...
  if (!strcmp (sm_prop->type, SmLISTofARRAY8))
  ...
  else if (!strcmp (sm_prop->type, SmARRAY8))
    {
      ...
      essm_properties_set_string (properties, sm_prop->name, sm_prop->vals[0].value);
    }
  else if (!strcmp (sm_prop->type, SmCARD8))
    {
      value_uchar = *(guchar *)(sm_prop->vals[0].value);
      essm_properties_set_uchar (properties, sm_prop->name, value_uchar);
    }
  else
    {
...
gboolean
essm_properties_remove (EssmProperties *properties,
                        const gchar *property_name)
{
  g_return_val_if_fail (properties != NULL, FALSE);
  g_return_val_if_fail (property_name != NULL, FALSE);

  essm_verbose ("-> Removing (%s)\n", property_name);
...
void
essm_properties_set_default_child_watch (EssmProperties *properties)
{
...
void
essm_properties_free (EssmProperties *properties)
{
  g_return_if_fail (properties != NULL);
  ...
  g_slice_free (EssmProperties, properties);
}
